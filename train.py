...
class Trainer(object):
    def __init__(self, model, optimizer, dataloader, graph, lr_scheduler, args, load_state=None):
        ...
        with open(self.time_log_file, 'w', encoding='utf-8') as f:
            f.write(f'[Timing Log] dataset={self.args.dataset}, epochs={self.args.epochs}, batch_size={self.args.batch_size}\n')
        ...
        self.training_stats = PD_Stats(
            ...
            ['epoch', 'train_loss', 'val_loss', 'train_pred_loss', 'train_variant_ssl', 'train_invariant_ssl',
             'val_pred_loss', 'val_variant_ssl', 'val_invariant_ssl'],
        )
    ...
    def train_epoch(self, epoch, loss_weights):
        self.model.train()
        total_loss = 0.0
        total_sep_loss = np.zeros(3, dtype=np.float64)
        ...
        for batch_idx, (data, data_m, target, time_label, c) in enumerate(self.train_loader):
            ...
            loss, sep_loss = self.model.calculate_loss(
                data, repr1, repr2, target, c, time_label, self.scaler, loss_weights, training=True
            )
            ...
            total_sep_loss += np.array(sep_loss, dtype=np.float64)
        ...
        self.logger.info(
            '*******Train Epoch {}: averaged Loss : {:.6f}, pred : {:.6f}, variant_ssl : {:.6f}, invariant_ssl : {:.6f}'.format(
                epoch, train_epoch_loss, total_sep_loss[0], total_sep_loss[1], total_sep_loss[2]
            )
        )
    ...
    def val_epoch(self, epoch, val_dataloader, loss_weights):
        ...
        with torch.no_grad():
            for batch_idx, (data, data_m, target, time_label, c) in enumerate(val_dataloader):
                repr1, repr2 = self.model(data, data_m)
                loss, sep_loss = self.model.calculate_loss(
                    data, repr1, repr2, target, c, time_label, self.scaler, loss_weights, training=False
                )
        ...
        self.logger.info(
            '*******Val Epoch {}: averaged Loss : {:.6f}, pred : {:.6f}, variant_ssl : {:.6f}, invariant_ssl : {:.6f}'.format(
                epoch, val_loss, total_sep_loss[0], total_sep_loss[1], total_sep_loss[2]
            )
        )

        return val_loss, total_sep_loss, val_time_s
    ...
    def train(self):
        ...
        loss_tm1 = np.ones(3, dtype=np.float64)
        loss_t = np.ones(3, dtype=np.float64)

        for epoch in range(1, self.args.epochs + 1):
            if self.args.use_dwa:
                loss_tm2 = loss_tm1.copy()
                loss_tm1 = loss_t.copy()
                if epoch == 1 or epoch == 2:
                    loss_weights = dwa(loss_tm1, loss_tm1, self.args.temp)
                else:
                    loss_weights = dwa(loss_tm1, loss_tm2, self.args.temp)
            else:
                loss_weights = np.ones(3, dtype=np.float64)

            self.logger.info('loss weights: {}'.format(loss_weights))

            train_epoch_loss, train_sep_loss, train_time_s = self.train_epoch(epoch, loss_weights)

            if train_epoch_loss > 1e6:
                self.logger.warning('Gradient explosion detected. Ending...')
                break

            val_epoch_loss, val_sep_loss, val_time_s = self.val_epoch(epoch, self.val_loader, loss_weights)
            ...
            self.training_stats.update([
                ...
                train_epoch_loss,
                val_epoch_loss,
                float(train_sep_loss[0]),
                float(train_sep_loss[1]),
                float(train_sep_loss[2]),
                float(val_sep_loss[0]),
                float(val_sep_loss[1]),
                float(val_sep_loss[2]),
            ])

            loss_t = train_sep_loss.copy()
        ...
        test_results = self.test(self.model, self.test_loader, self.scaler, self.graph, self.logger, self.args)
    ...
    def test(model, dataloader, scaler, graph, logger, args):
        ...
        with torch.no_grad():
            for batch_idx, (data, data_m, target, time_label, c) in enumerate(dataloader):
                ...
                reps_i.append(repr1.detach().cpu())
                reps_v.append(repr2.detach().cpu())
                alpha_list.append(alpha.detach().cpu())
                beta_list.append(beta.detach().cpu())

        y_true = scaler.inverse_transform(torch.cat(y_true, dim=0))
        y_pred = scaler.inverse_transform(torch.cat(y_pred, dim=0))
        reps_i = torch.cat(reps_i, dim=0)
        reps_v = torch.cat(reps_v, dim=0)
        ...
        np.savez(rep_path, RI=reps_i.cpu().numpy(), RV=reps_v.cpu().numpy())
